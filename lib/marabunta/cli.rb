...
USAGE
    ...
    desc "setup [CONFIGURATION]", "sets up murder's configuration files through the cloud nodes"
    def setup(configuration = "#{Dir.pwd}/Leiningen")
      config = load_config configuration

      Marabunta::Deployer.new(config).setup!
    end

    private
    def load_config(configuration)
      config = instance_eval(File.read(configuration), configuration)
      validate! config
